...
import { TOPICS, getAdapter, type TopicKey } from "./nextjs-adapters";

const NextjsControls: React.FC = () => {
  const dispatch = useDispatch();
  const st = useSelector((state: RootState) => state.nextjs) as NextjsState;
  ...
  return (
    <div className="nextjs-controls">
      {/* ── Topic dropdown ── */}
      <div className="nextjs-controls__group">
        <select
          className="nextjs-controls__select"
          value={st.topic}
          onChange={(e) =>
      ...
      <span className="nextjs-controls__sep" />
      ...
      <div className="nextjs-controls__group">
        {activeTopic.variants.map((vk: VariantKey) => {
          ...
          return (
            <button
              key={vk}
              className={`nextjs-controls__btn${st.variant === vk ? " nextjs-controls__btn--active" : ""}`}
              style={
                st.variant === vk
                  ? {
