...
export const useFailoverAnimation = (onAnimationComplete?: () => void) => {
  ...
  useEffect(() => {
    ...
    const doPatch = (p: Partial<FailoverState>) => dispatch(patchState(p));
    ...
    const run = async () => {
      ...
      if (stepDef.action === "failPrimary") {
        dispatch(failPrimary());
      }
      if (stepDef.action === "promoteSecondary") {
        dispatch(promoteSecondary());
      }
      if (stepDef.action === "completeFailover") {
        dispatch(completeFailover());
      }
