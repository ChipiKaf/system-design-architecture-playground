...
export const VARIANT_PROFILES: Record<ProtocolKey, VariantProfile> = {
  ...
  mqtt: {
    ...
    color: "#a78bfa",
    description:
      "AWS-native MQTT for device fleets: devices stay connected to AWS IoT Core, send readings on named message channels, and let AWS route, store, and send commands without the device calling backend services directly.",
    format: "MQTT packets + JSON payload",
    coupling: "loose",
    strengths: [
      "Works well for small devices and unreliable networks",
      "One long-lived connection can send data up and receive commands down",
      "The broker keeps devices decoupled from backend services",
      "AWS has built-in routing and device-state features for common IoT flows",
    ],
    weaknesses: [
      "No replayable log like Kafka",
      "Each device needs its own identity and permissions",
      "Topic names need a clean design or fleets become hard to manage",
      "Best for device messaging, not for every backend integration problem",
      "You still need good fleet setup for onboarding, updates, and monitoring",
    ],
    awsServices: [
      {
        name: "AWS IoT Core",
        role: "The managed front door that devices connect to and exchange messages through",
      },
      {
        name: "IoT Rules Engine",
        role: "If a message arrives on the right topic, forward it to other AWS services",
      },
      {
        name: "Lambda",
        role: "Small cloud code that can check or reshape a device message",
      },
      {
        name: "Timestream",
        role: "Database for readings collected over time, like temperature every minute",
      },
      {
        name: "Device Shadow",
        role: "A cloud copy of what the device last said and what the cloud wants it to become",
      },
      {
        name: "SQS Dead-Letter Queue",
        role: "Safety net for messages that fail processing — preserved for inspection and replay",
      },
    ],
    nodes: [
      "device-temp",
