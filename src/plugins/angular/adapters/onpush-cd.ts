...
/* ── Positions: OnPush — targeted checking ───────────── */
const POS = {
  zonejs: { x: 480, y: 55 },
  event: { x: 160, y: 55 },
  cd: { x: 480, y: 175 },
...
export const onpushCdAdapter: AngularAdapter = {
  id: "onpush-cd",

  profile: {
    label: "OnPush Strategy",
    description:
      "OnPush — Angular only checks a component when its @Input() reference changes, an Observable emits via async pipe, or markForCheck() is called. Unchanged branches are skipped entirely.",
  },
  ...
  computeMetrics(state: AngularState) {
    ...
    const active =
      ...
      p === "cd-check" ||
      p === "cd-summary";
    state.cdStrategy = active ? "onpush" : "none";
    state.checksPerCycle = active ? "targeted" : "none";
    state.skipCount = active ? 2 : 0;
  },
    ...
    return [
      {
        ...
        color: "#22c55e",
        explain:
          "✓ Only ComponentA checked — B and C skipped (no @Input change).",
      },
    ];
  },
  ...
  buildTopology(builder: any, state: AngularState, helpers) {
    ...
    const active = state.cdStrategy === "onpush";
    ...
    /* ─ compA: Component A — @Input changed → checked ─ */
    builder
      .node("compA")
      ...
      .richLabel(
        (l: any) => {
          ...
          l.newline();
          l.color(
            active ? "✓ @Input changed → checked" : "@Input changed",
    ...
    /* ─ compB: Component B — OnPush, skipped ─ */
    builder
      .node("compB")
      .at(POS.compB.x, POS.compB.y)
      .rect(200, 52, 12)
      .fill(hot("compB") ? "#1c1917" : "#0f172a")
      .stroke(hot("compB") ? "#64748b" : "#334155", 2)
      .richLabel(
        (l: any) => {
          l.bold("ComponentB");
          l.newline();
          l.color(
            active ? "⏭ SKIPPED — OnPush" : "OnPush, no @Input change",
            active ? "#94a3b8" : "#94a3b8",
            { fontSize: 9 },
          );
        },
        {
          fill: active ? "#64748b" : "#fff",
          fontSize: 11,
          dy: -2,
          lineHeight: 1.6,
        },
    ...
    /* ─ compC: Component C — OnPush, skipped ─ */
    builder
      .node("compC")
      .at(POS.compC.x, POS.compC.y)
      .rect(200, 52, 12)
      .fill(hot("compC") ? "#1c1917" : "#0f172a")
      .stroke(hot("compC") ? "#64748b" : "#334155", 2)
      .richLabel(
        (l: any) => {
          l.bold("ComponentC");
          l.newline();
    ...
    builder
      ...
      .richLabel(
        (l: any) => {
          l.color(
            active
              ? "✓ Only 1/3 checked — 2 skipped, faster cycle"
              : "Only changed branches are checked",
            active ? "#86efac" : "#94a3b8",
            { fontSize: 9 },
          );
        },
    ...
    builder
      ...
      .arrow(true)
      .label("4. ✓ Check", { fill: "#86efac", fontSize: 9 });

    builder
      .edge("root", "compB", "e-root-b")
      .stroke(hot("compB") ? "#64748b" : "#475569", 2)
      .arrow(true)
      .dashed(true)
      .label("4. ⏭ Skip", { fill: "#94a3b8", fontSize: 9 });

    builder
      .edge("root", "compC", "e-root-c")
      .stroke(hot("compC") ? "#64748b" : "#475569", 2)
      .arrow(true)
      .dashed(true)
      .label("4. ⏭ Skip", { fill: "#94a3b8", fontSize: 9 });
  ...
  getStatBadges(state: AngularState) {
    return [
      { label: "Strategy", value: "OnPush", color: "#22c55e" },
      {
        label: "Checks",
        value: state.checksPerCycle === "targeted" ? "1/3" : "—",
        color: state.checksPerCycle === "targeted" ? "#22c55e" : "#64748b",
      },
      {
        label: "Skipped",
        value: String(state.skipCount),
        color: state.skipCount > 0 ? "#f59e0b" : "#64748b",
      },
      { label: "Trigger", value: "@Input / async", color: "#a78bfa" },
    ];
  },
